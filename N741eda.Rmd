...
author: "Julianne Ammirati, Editing Code by Dr. Melinda Higgins"
...
```{r}
head(cmc) #Head means start at the top of the dataset, tail would be to start at bottom
```

### Print this subset using `knitr::kable()`

```{r}
knitr::kable(head(cmc)) #KABLE will generate a table
```
...
```{r}
mean(cmc$HusbEd) #this won't work because husband's ed has to be recoded as numeric
...
# initial steps
sum1 <-cmc %>% #this will write the ouput to new object
#cmc %>% #take the dataset CMC and then summarize the following:
...
cmc %>%
    ...
    summarise(nChild = length(NumChild),
              ...
              meanChild = mean(NumChild),
              sdChild = sd(NumChild),
              pct75 = quantile(NumChild, 0.75), #will give you 75% percentile
              medianChild = median(NumChild),
              maxChild = max(NumChild)) %>%
...
cmc %>%
  ggplot(aes(WifeAge)) +
    geom_histogram(aes(y=..density..),
                   ...
                   binwidth=5) +
    geom_density() #nonparametric density approach
```
